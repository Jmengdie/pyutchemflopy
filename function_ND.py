...
def function_ND(P0, P1, P2):
    ...
    # # 指定保存生成文件的目录
    # output_dir = r'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\utchem'
    #
    # # # 第一阶段
    # # # 指定输入文件路径(HEAD和INPUT文件)
    # head_path = r'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\utchem\head'
    # input_path = r'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\utchem\input'
    #
    # # 如果输出目录不存在，则创建该目录
    ...
    # infile_path = r'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\utchem\infile'
    ...
    #     filename = r'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\utchem\UTex21.SATP'
    #     # 关键词1：提取结果所在的时间
    ...
    #     heatmap_filename = fr'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\utchem\Sn_lay_{i}.txt'
    ...
    # # 指定输入文件路径(HEAD和INPUT文件)
    # input_file_name = r'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\UTex21.SATP'
    # mesh_file_name = r'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\UTex21.MESH'
    #
    # # 指定剖面参数X-Z or Y-Z
    # parameter = 'X-Z'
    #
    ...
    # # i表示行数
    # for i in [20, 25, 30]:
    #     filename = r'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\UTex21_XZ.SATP'
    #     # 关键词1：提取结果所在的时间
    ...
    #
    #     # Save the heatmap data to a text file
    #     heatmap_filename = fr'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test\Sn_row_{i}.txt'
    #     np.savetxt(heatmap_filename, heatmap_data, fmt='%.4f', delimiter='\t')
    #
    #     # 对应每一个地层平面绘图
    #     plt.imshow(heatmap_data, cmap='viridis', interpolation='nearest')
    ...
    for i in [1, 2, 3, 4, 5, 6, 7, 8, 9]:
        Sn = np.loadtxt(
            fr'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test3\utchem3\Sn_lay_{i}.txt')
    ...
    # DSP
    dsp = flopy.mt3d.Mt3dDsp(mt, al=P[2], dmcoef=0, trpt=0.1, trpv=0.01)
    ...
    beta = 0.05 ** 0.61
    gamma = 1.24
    D = P[0]  # 扩散系数（分子扩散系数），单位为m2/s；6.5E-10~8.5E-10
    L = P[1]  # 特征长度，单位为m；粘土或细粒介质，特征长度范围10E-6~10E-4之间；场地规模地下水模型的计算网格尺度，1~10m之间
    ...
    for per in range(nper):  # nper 是应力期数量
        for i in range(nlay):
            Sn = np.loadtxt(
                fr'H:\BaiduSyncdisk\08researchprogress\04Simulation\01_JiaDing_test\pyutchemflopy_Jiading_test3\utchem3\Sn_lay_{i + 1}.txt')
            ...
            for row in range(min(nrow, Sn_nrow)):
                for col in range(min(ncol, Sn_ncol)):
                    Sn_value = Sn[row, col]
                    if Sn_value != 0:
                        current_flux = alpha * beta * (Sn_value ** gamma) * D / L * (
                                    C_solubility - C_water[i, row, col])
...
function_ND(7.31113347e-07, 9.32896229e+00, 2.17777838e+01)
