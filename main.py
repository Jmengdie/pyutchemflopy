...
class SimulationThread(QThread):
    ...
    def run(self):
        # 模拟运行代码，包括生成文件的操作
        pyutchemflopy_model(self.file_path, self.parameters)
...
class CustomGraphicsView(QGraphicsView):
    def __init__(self):
        ...
        self.setMinimumSize(0, 0)  # 设置最小尺寸，根据需要调整
    ...
    def Sn_XY(self, num):
        ...
        lay = num
        heatmap_data, heatmap_filename = Sn_XY(lay)

        # 将 Matplotlib 图像嵌入到 Qt 中
        fig, ax = plt.subplots()
        im = ax.imshow(heatmap_data, cmap='viridis', interpolation='nearest')
        ax.set_title(f'Sn of DNAPLs in lay{lay}')

        # 添加颜色条
        cbar = fig.colorbar(im, ax=ax, shrink=0.5)
        cbar.set_label('Sn')  # 设置颜色条的标签，根据实际情况自行修改
    ...
    def Sn_XZ(self, num):
        # 调用 result_plot.py 中的 draw_sn_xy_result 函数
        row = num
        heatmap_data, heatmap_filename = Sn_XZ(row)

        # 将 Matplotlib 图像嵌入到 Qt 中
        fig, ax = plt.subplots()
        im = ax.imshow(heatmap_data, cmap='viridis', interpolation='nearest')
        ax.set_title(f'Sn of DNAPLs in row{row}')
    ...
    def Sn_YZ(self, num):
        # 调用 result_plot.py 中的 draw_sn_xy_result 函数
        col = num
        heatmap_data, heatmap_filename = Sn_YZ(col)

        # 将 Matplotlib 图像嵌入到 Qt 中
        fig, ax = plt.subplots()
        im = ax.imshow(heatmap_data, cmap='viridis', interpolation='nearest')
        ax.set_title(f'Sn of DNAPLs in col{col}')
    ...
    def head_plot(self):
        head_data = head_plot(file_path)

        # 将 Matplotlib 图像嵌入到 Qt 中
        fig, ax = plt.subplots(figsize=(12, 8))
        head_p = ax.imshow(head_data)
        ax.set_title('Head (m)')
    ...
    def conc_XY(self, num):
        # 调用 result_plot.py 中的 draw_sn_xy_result 函数
        conc_data = conc_XY(num, conc)

        # 将 Matplotlib 图像嵌入到 Qt 中
        fig, ax = plt.subplots(figsize=(12, 8))
        head_p = ax.imshow(conc_data)
        ax.set_title(f'Cn of DNAPLs in lay{num}')
        plt.colorbar(head_p, fraction=0.05, pad=0.05, shrink=0.3)
    ...
    def conc_XZ(self, num):
        # 调用 result_plot.py 中的 draw_sn_xy_result 函数
        conc_data = conc_XZ(num, conc)

        # 将 Matplotlib 图像嵌入到 Qt 中
        fig, ax = plt.subplots(figsize=(12, 8))
        head_p = ax.imshow(conc_data)
        ax.set_title(f'Cn of DNAPLs in row{num}')
        plt.colorbar(head_p, fraction=0.05, pad=0.05, shrink=0.3)
    ...
    def conc_YZ(self, num):
        # 调用 result_plot.py 中的 draw_sn_xy_result 函数
        conc_data = conc_YZ(num, conc)

        # 将 Matplotlib 图像嵌入到 Qt 中
        fig, ax = plt.subplots(figsize=(12, 8))
        head_p = ax.imshow(conc_data)
        ax.set_title(f'Cn of DNAPLs in col{num}')
        plt.colorbar(head_p, fraction=0.05, pad=0.05, shrink=0.3)
    ...
    def __init__(self):
        ...
        for child in self.ui.stackedWidget.widget(2).children():
            if isinstance(child, QGraphicsView):
                ...
                # 获取 graphicsView 的位置和大小
                geometry = child.geometry()
                # 将这些信息应用到 CustomGraphicsView
                self.customGraphicsView.setGeometry(geometry)
                # 设置 CustomGraphicsView 为 stackedWidget 的主要组件
                self.ui.stackedWidget.widget(2).layout().addWidget(self.customGraphicsView)
                # 移除原始的 graphicsView
                self.ui.stackedWidget.widget(2).layout().removeWidget(child)
